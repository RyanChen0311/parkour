...
- **殘影特效**：移動時產生炫酷的幻影拖尾
- **🏃 開始跑酷**：自動移動模式，背景轉黑，每 1.5 秒隨機換色
- **🎲 隨機職業**：自動輪替職業，每次切換同步隨機換色
- **速度控制**：長條滑桿調整移動速度
...
| 開始跑酷加入自動換色 | 每 1.5 秒隨機換色（`autoColorLoop`） |
| 隨機職業換色 | `autoChangeClass` 切換時同步隨機換色 |
...
| 按鈕顯示「自動選擇職業」 | 統一改為「隨機職業」（HTML 初始值 + JS toggle） |
